...
{syntab:Main}
{synopt:{opt sa:ve(filename)}*}name of the new .docx file to be saved.{p_end}
...
{synopt:{opt t:itle(string)}}survey title for Word document.{p_end}
{synopt:{opt sub:title(string)}}survey subtitle for Word document.{p_end}
{synopt:{opt p:agesize(string)}} {help putdocx} page size for Word.{p_end}
{synopt:{opt scheme(string)}}set colors used in the final document.{p_end}
{synopt:{opt int:length(int)}}number of spaces provided for {it:integer} variable inputs.{p_end}
{synopt:{opt dec:length(int)}}number of spaces provided for {it:decimal} variable inputs.{p_end}
{synopt:{opt split}}create a new table in the Word document for every top-level group.{p_end}
{synopt:{opt max:heading(int)}}maximum number of nested group-levels to keep.{p_end}
{synopt:{opt keepr:elevance}}edits and display {it:relevance} expressions in the document.{p_end}
...
to construct an editable Microsoft Word document with appropriately-formated
tables containing the variable names or numbers, the question asked ({it:labels}), 
hints, skip patterns (via {it:relevance} expressions), and choice lists.

{pstd}
The resulting Word document contains a table/tables with headings for each group 
(with a cascading style similar to Word's default Heading 1, Heading 2,etc. 
for nested groups) and three columns for each line in the {it:survey}
sheet: {p_end}
{p2colset 8 12 12 4}
